...
- [ ] Add failing strict-schema and sparse-WAV tests expecting
...
- [ ] Run the new tests and confirm they fail on the old `audio_sha256` full
  file hash.
- [ ] Add a bounded RIFF/WAVE parser with fixed read-byte and chunk-count
  budgets; include only canonical persisted metadata in its SHA-256.
- [ ] Bump the strict plan version and rename all plan/record/schema identity
...
- [ ] Add failing instrumentation tests that pause prescan and prove an
  unrelated `BEGIN IMMEDIATE` writer commits without waiting for the five-second
  busy timeout.
- [ ] Add a failing guard test that raises if any file snapshot/stat operation
  occurs after enqueue starts its SQLite write transaction.
- [ ] Replace transaction-time `_build_plan` file reads with a root-locked
  `_FilesystemScan` created before `BEGIN`; load jobs and repairs once and build
  only from those immutable in-memory views.
- [ ] Use `BEGIN` explicitly for plan reads and `BEGIN IMMEDIATE` only after
  enqueue prescan/revalidation; commit both before releasing root EX.
- [ ] Run concurrency, lock-order, idempotency, and bounded-FD tests to green.
...
- [ ] Add failing real-old-table tests for `PRAGMA notnull=1`, FK/index
  preservation, non-null source backfill, and null-source/null-audio runnable
  rows becoming deterministic permanent failures.
- [ ] Add a single-transaction rename/create/copy/drop rebuild that maps
...
- [ ] Run migration twice and assert identical rows/schema on the second call.
- [ ] Run migration and store contract tests to green.
...
- [ ] Add a failing race test that renames/replaces the parent immediately after
  the second binding check returns.
- [ ] Add final parent binding and target inode/regular-file/0600 verification
  after link, temporary unlink, and parent fsync, immediately before clearing
  cleanup identity and returning.
- [ ] Assert cleanup through the held parent fd removes only the invocation's
  final inode from the moved directory.
- [ ] Run all private-plan safety tests to green.
...
- [ ] Add failing report assertions for `scan_entries` and
  `audio_probe_max_bytes`, with no elapsed time in the plan digest.
- [ ] Populate the deterministic fields from the complete allowlist scan and
  render them without paths or subtitle text.
- [ ] Run historical/store focused tests, then full pytest, compileall, and
  `git diff --check`.
- [ ] Inspect the final diff for file I/O after `BEGIN`, schema false claims,
