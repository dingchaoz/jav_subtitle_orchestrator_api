...
def _delete_job_with_request_history(store: JobStore, job_id: str) -> None:
    with store.connection() as conn:
        conn.execute("DELETE FROM callback_events WHERE job_id = ?", (job_id,))
        conn.execute("DELETE FROM job_requests WHERE job_id = ?", (job_id,))
...
def test_execute_rechecks_receipt_and_classifies_changed_without_sync(
    ...
    plan = _one_missing_plan(tmp_path, store, job)
    if change == "deleted":
        _delete_job_with_request_history(store, job.id)
    else:
        with store.connection() as conn:
            if change == "not_ready":
                conn.execute(
